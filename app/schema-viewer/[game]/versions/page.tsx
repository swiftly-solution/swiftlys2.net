...
export default async function SchemaVersionsPage(
    props: PageProps<"/schema-viewer/[game]/versions">,
) {
    const { game: gameId } = await props.params;
    const baseUrl = await getBaseUrl();
    const res = await fetch(`${baseUrl}/api/schema/versions?game=${gameId}`);
        ...
        <div className="space-y-6">
            {versions.length > 1 && (
                <div className="rounded-2xl border border-white/10 bg-zinc-950/40 p-6">
                    ...
                    <form
                        action={`/schema-viewer/${gameId}/diff`}
                        method="get"
                ...
                {versions.map((version, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {previous && (
                                <a
                                    href={`/schema-viewer/${gameId}/diff?from=${previous.sha}&to=${version.sha}`}
