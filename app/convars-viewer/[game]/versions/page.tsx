...
export default async function ConvarsVersionsPage(
    props: PageProps<"/convars-viewer/[game]/versions">,
) {
    const { game: gameId } = await props.params;
    const baseUrl = await getBaseUrl();
    const res = await fetch(`${baseUrl}/api/convars/versions?game=${gameId}`);
        ...
        <div className="space-y-6">
            {versions.length > 1 && (
                <div className="rounded-2xl border border-white/10 bg-zinc-950/40 p-6">
                    ...
                    <form
                        action={`/convars-viewer/${gameId}/diff`}
                        method="get"
                ...
                {versions.map((version, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {previous && (
                                <a
                                    href={`/convars-viewer/${gameId}/diff?from=${previous.sha}&to=${version.sha}`}
